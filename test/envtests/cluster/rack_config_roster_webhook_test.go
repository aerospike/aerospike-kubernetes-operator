...
var _ = Describe("RackConfig maxIgnorablePods and forceBlockFromRoster deploy validation", func() {
	ctx := context.TODO()
	...
	Context("spec.rackConfig.maxIgnorablePods", func() {
		Context("positive", func() {
			// ENV-FPH-010: maxIgnorablePods=0 is valid (no pods ignorable; not treated as negative).
		...
		Context("negative", func() {
			...
			It("rejects create with an invalid maxIgnorablePods IntOrString value", func() {
				...
				// validateIntOrStringField returns intstr.GetScaledValueFromIntOrPercent error
				// (no field path in that error); admission wraps with webhook prefix.
				envtests.NewStatusErrorMatcher().
					WithMessageSubstrings(
						testutil.WebhookErrorPrefix,
