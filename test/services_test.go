...
var _ = Describe("ClusterService", func() {
	ctx := goctx.TODO()
	It("Validate LB created", func() {
		By("DeployCluster with LB")
		clusterNamespacedName := getClusterNamespacedName("load-balancer-create", namespace)
	...
	It("Validate LB can not be updated", func() {
		By("DeployCluster")
		clusterNamespacedName := getClusterNamespacedName("load-balancer-invalid", namespace)
		aeroCluster := createDummyAerospikeCluster(clusterNamespacedName, 2)
		aeroCluster.Spec.SeedsFinderServices.LoadBalancer = createLoadBalancer()
		err := deployCluster(k8sClient, ctx, aeroCluster)
		Expect(err).ToNot(HaveOccurred())
	...
	It("Validate LB created in existing cluster", func() {
		By("DeployCluster without LB")
		clusterNamespacedName := getClusterNamespacedName("load-balancer-update", namespace)
		aeroCluster := createDummyAerospikeCluster(clusterNamespacedName, 2)
...
func createLoadBalancer() *asdbv1beta1.LoadBalancerSpec {
	...
	Expect(validCloud).To(BeTrue(), fmt.Sprintf("Can't find LoadBalancer specification for cloud provider \"%s\"", cloudProvider))
...
func loadBalancerName(aeroCluster *asdbv1beta1.AerospikeCluster) types.NamespacedName {
	return types.NamespacedName{Name: aeroCluster.Name + "-lb", Namespace: aeroCluster.Namespace}
}
