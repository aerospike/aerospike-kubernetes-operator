	...
	"AccessControl", func() {
		...
		Context(
			"AccessControl", func() {
				...
				// User name validation (empty, whitespace-only, >63 chars, containing ":" or ";") is now
				// enforced at the CRD schema level via kubebuilder markers (MinLength=1, MaxLength=63,
				// Pattern) rather than in IsAerospikeAccessControlValid. Those cases are covered by
				// the webhook envtests in test/envtests/cluster/access_control_webhook_test.go.

				// Role name validation (empty, whitespace-only, >63 chars, containing ":" or ";") is
				// now enforced at the CRD schema level via kubebuilder markers (MinLength=1, MaxLength=63,
				// Pattern) rather than in IsAerospikeAccessControlValid. Those cases are covered by
				// the webhook envtests in test/envtests/cluster/access_control_webhook_test.go.
