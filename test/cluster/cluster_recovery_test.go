...
// waitForClusterPodsFailedOutsideGrace waits until every cluster pod is in a failed state, then
// sleeps through the failed-pod grace window so hasClusterFailed() can return true.
func waitForClusterPodsFailedOutsideGrace(
	ctx goctx.Context, c client.Client, nn types.NamespacedName, size int32,
) error {
	timeout := getTimeout(size)

	err := wait.PollUntilContextTimeout(ctx, retryInterval, timeout, true, func(ctx goctx.Context) (bool, error) {
		...
		if len(pl.Items) < int(size) {
			return false, nil
		}

		for i := range pl.Items {
			pod := &pl.Items[i]
			if oputils.CheckPodFailedWithGrace(pod, true).State == oputils.PodHealthy {
				return false, nil
			}
		}
	...
	Context("Deploy Validation", func() {
		...
		Context("When failed-create recovery is triggered", func() {
			...
			It("recovers after bad image rollout when stale ACL remains in status and spec image is corrected", func() {
				...
				Expect(waitForClusterPodsFailedOutsideGrace(ctx, k8sClient, clusterNamespacedName, 2)).To(Succeed())
				...
				ac.Spec.Image = validImage
				Expect(updateClusterWithNoWait(k8sClient, ctx, ac)).To(Succeed())
