...
var _ = FDescribe("CrossRackBatchQuiesce", func() {
	...
	Context("A remaining (non-target) pod in a scaled-down rack crashes", func() {
		...
		It("Should block scale-down until the remaining pod recovers", func() {
			aeroCluster, err := getCluster(k8sClient, ctx, clusterNamespacedName)
			Expect(err).ToNot(HaveOccurred())

			// Identify the pod that will REMAIN on rack 1 after scale-down to
			// size 4 (i.e. pod-0, the lowest-index pod).
			rack1Pods := podNamesForRack(ctx, aeroCluster, 1)
			Expect(len(rack1Pods)).To(BeNumerically(">=", 2))
			remainingPodName := rack1Pods[0] // lowest index — will NOT be removed

			// Step 1: induce a sustained crash on the remaining pod by killing
			// its aerospike server process several times in rapid succession.
			// Each kill increments the container's restart count, triggering
			// Kubernetes' exponential backoff (≥ 10 s between restarts after
			// the second crash).  This keeps the pod not-ready long enough for
			// the operator to observe the failure — without needing any
			// explicit recovery action from the test.
			By("Crashing remaining pod to trigger CrashLoopBackOff")

			for i := 0; i < 3; i++ {
				// Ignore errors: the container may be restarting mid-loop.
				_ = execInPodContainer(
					clusterNamespacedName.Namespace, remainingPodName,
					asdbv1.AerospikeServerContainerName,
					[]string{"kill", "-9", "1"},
				)
			...
			// Step 2: wait until the remaining pod is confirmed NOT
			// server-ready before triggering scale-down.  This guarantees
			// checkReadyForBatchQuiesce will see a not-running pod with CR
			// status → ReconcileError.
			By("Waiting for remaining pod to be not-server-ready")
			Eventually(func() bool {
				p := &corev1.Pod{}
				if getErr := k8sClient.Get(ctx, types.NamespacedName{
					Name:      remainingPodName,
					Namespace: clusterNamespacedName.Namespace,
				}, p); getErr != nil {
					return true // pod temporarily absent during restart
				}

				return !utils.IsAerospikeServerReady(p)
			}, 2*time.Minute, 2*time.Second).Should(BeTrue(),
				"Remaining pod should be not-ready after crash")

			// Step 3: trigger scale-down now — the remaining pod is down.
			aeroCluster, err = getCluster(k8sClient, ctx, clusterNamespacedName)
			Expect(err).ToNot(HaveOccurred())
			...
			// Step 4: operator must enter Error because the remaining pod has
			// CR status (it joined before) but is not running now.
			By("Confirming operator enters Error phase")
			Eventually(func() asdbv1.AerospikeClusterPhase {
				cl, gErr := getCluster(k8sClient, ctx, clusterNamespacedName)
				if gErr != nil {
					return ""
				...
				Equal(asdbv1.AerospikeClusterError),
				"Operator should be in Error while remaining pod is not running",
			)

			// Step 5: no explicit recovery needed — the CrashLoopBackOff
			// backoff timer expires and Kubernetes restarts the container
			// automatically.  Once the remaining pod is Running again, the
			// operator retries and completes scale-down.
			By("Waiting for scale-down to complete after pod recovery")
			Expect(waitForAerospikeCluster(
				k8sClient, ctx, aeroCluster, int(aeroCluster.Spec.Size),
				retryInterval, getTimeout(8),
				[]asdbv1.AerospikeClusterPhase{asdbv1.AerospikeClusterCompleted},
