...
var _ = Describe("SecurityContext", func() {
	ctx := goctx.TODO()
	It("Validate SecurityContext applied", func() {
		By("DeployCluster with SecurityContext")
		clusterNamespacedName := getClusterNamespacedName("security-context-create", namespace)
	...
	It("Validate SecurityContext updated", func() {
		By("DeployCluster")
		clusterNamespacedName := getClusterNamespacedName("security-context-updated", namespace)
		aeroCluster := createDummyAerospikeCluster(clusterNamespacedName, 2)
...
func validateSecurityContext(aeroCluster *asdbv1beta1.AerospikeCluster) {
	pods, err := getPodsList(k8sClient, goctx.TODO(), getClusterNamespacedName(aeroCluster.Name, aeroCluster.Namespace))
	Expect(err).ToNot(HaveOccurred())
