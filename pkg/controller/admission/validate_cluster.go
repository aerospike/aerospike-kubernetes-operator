...
// 	// TODO: No need if its enterprise cluster.
// 	// if !isEnterprise(s.obj.Spec.Build) && s.obj.Spec.Size > maxCommunityClusterSz {
// 	// 	return fmt.Errorf("CommunityEdition Cluster size cannot be more than %d", maxCommunityClusterSz)
// 	// }
...
// 				blockStorageDeviceList = append(blockStorageDeviceList, device.Name)
// 			}
// 		}
// 	}

// 	var fileStorageMountPath string
// 	if s.obj.Spec.AerospikeConfig["namespace"] == nil {
...
// 								return fmt.Errorf("Namespace storage-device %v not found in BlockStorage config %v", dev, s.obj.Spec.BlockStorage)
// 							}
// 						}
// 						logger.Debug("Valid namespace device", log.Ctx{"device": device})
// 					}
// 				}
// 				if filesInterface, ok := storage.(map[string]interface{})["file"]; ok {
// 					// Namespace fileSystem should have FileStorage config section
// 					if s.obj.Spec.FileStorage.StorageClass == "" {
// 						return fmt.Errorf("No FileStorage confing given for namespace fileSystem %v", storage)
// 					}
// 					if filesInterface == nil {
// 						return fmt.Errorf("namespace storage files cannot be nil %v", storage)
// 					}
// 					if _, ok := filesInterface.([]interface{}); !ok {
// 						return fmt.Errorf("namespace storage files format not valid %v", filesInterface)
// 					}
// 					if len(filesInterface.([]interface{})) == 0 {
// 						return fmt.Errorf("No files for namespace storage %v", filesInterface)
// 					}
// 					// Files should have common mountPath
// 					for _, file := range filesInterface.([]interface{}) {
// 						dirPath := filepath.Dir(file.(string))
// 						if fileStorageMountPath == "" {
// 							fileStorageMountPath = dirPath
// 							continue
// 						}
// 						if fileStorageMountPath != dirPath {
// 							return fmt.Errorf("Namespace storage-engine files should have common mountPath. Mismatched mountPath found %s and %s", fileStorageMountPath, dirPath)
// 						}
// 					}
// 				}
// 			} else {
...
// 	if res != nil && res.Limits != nil && res.Requests != nil &&
