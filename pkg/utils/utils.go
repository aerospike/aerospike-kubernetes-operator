...
import (
	...
	"golang.org/x/crypto/ripemd160" //nolint:gosec,staticcheck // this ripemd160 legacy hash is only used for diff comparison not for security purpose
...
func GetHash(str string) (string, error) {
	...
	//nolint:gosec // this ripemd160 legacy hash is only used for diff comparison not for security purpose
	hash := ripemd160.New()
	hash.Reset()
