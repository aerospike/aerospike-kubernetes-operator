...
func (r *SingleClusterReconciler) deleteLocalPVCs(rackState *RackState, pod *corev1.Pod) error {
	pvcItems, err := r.getPodsPVCList([]string{pod.Name}, rackState.Rack.ID, rackState.Rack.RackSuffix)
	if err != nil {
...
func (r *SingleClusterReconciler) getRackPVCList(rackID int, rackSuffix string) (
	[]corev1.PersistentVolumeClaim, error,
) {
	// List the pvc for this aeroCluster's statefulset
	pvcList := &corev1.PersistentVolumeClaimList{}
