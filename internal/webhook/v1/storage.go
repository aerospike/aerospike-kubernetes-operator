	...
	for newVolIdx := range newStorage.Volumes {
		...
		for oldVolIdx := range oldStorage.Volumes {
			...
			if oldVolume.Name == newVolume.Name {
				if !isSafeChange(newVolume, oldVolume) {
...
func isSafeChange(newVolume, oldVolume *asdbv1.VolumeSpec) bool {
...
func validateStorageVolumeSource(volume *asdbv1.VolumeSpec) error {
	...
	if source.PersistentVolume != nil {
		...
		if source.PersistentVolume.VolumeMode == v1.PersistentVolumeBlock {
			if volume.InitMethod == asdbv1.AerospikeVolumeMethodDeleteFiles {
				return fmt.Errorf(
			...
			if volume.WipeMethod != asdbv1.AerospikeVolumeMethodBlkdiscard &&
				volume.WipeMethod != asdbv1.AerospikeVolumeMethodDD {
			...
			if volume.InitMethod != asdbv1.AerospikeVolumeMethodNone &&
				volume.InitMethod != asdbv1.AerospikeVolumeMethodDeleteFiles {
				return fmt.Errorf(
			...
			if volume.WipeMethod != asdbv1.AerospikeVolumeMethodDeleteFiles {
				return fmt.Errorf(
