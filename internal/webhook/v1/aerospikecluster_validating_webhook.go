...
func validate(aslog logr.Logger, cluster *asdbv1.AerospikeCluster) (admission.Warnings, error) {
	...
	if err := validateClientCertSpec(
		cluster.Spec.OperatorClientCertSpec, cluster.Spec.AerospikeConfig,
	); err != nil {
		return warnings, err
	}

	if err := validateNetworkPolicy(cluster.Namespace, cluster); err != nil {
		return warnings, err
	}
...
func validateClientCertSpec(
	...
	networkConf, networkConfExist := configSpec.Value[asdbv1.ConfKeyNetwork]
	if !networkConfExist {
		return nil
	}
	...
	case reflect.DeepEqual("any", tlsAuthenticateClientConfig):
		...
		if err := validateOperatorClientCert(clientCertSpec); err != nil {
			return err
		}
...
func validateNetworkPolicy(namespace string, cluster *asdbv1.AerospikeCluster) error {
