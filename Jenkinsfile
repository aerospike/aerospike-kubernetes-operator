                    ...
                    steps {
                        script {
                           dir("${env.GO_REPO}") {
                               ...
                               sh "./snyk-linux test  --severity-threshold=high --fail-on=upgradable"

                               // Scan the operator images
                               sh "./snyk-linux container test ${OPERATOR_CONTAINER_IMAGE_CANDIDATE_NAME} --severity-threshold=high --file=Dockerfile --policy-path=.snyk --fail-on=upgradable"
                               sh "./snyk-linux container test ${OPERATOR_BUNDLE_IMAGE_CANDIDATE_NAME} --severity-threshold=high --file=Dockerfile --policy-path=.snyk --fail-on=upgradable"
                           }
                        }
                    }
                }
